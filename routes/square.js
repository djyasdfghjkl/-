...
router.get('/square/recommend', optionalAuth, async (ctx) => {
  try {
    ...
    ctx.status = 500;
    ctx.body = { success: false, message: '获取推荐日记失败：' + error.message };
  }
});
...
router.get('/square/hot', optionalAuth, async (ctx) => {
  try {
    ...
    console.error('[获取热门日记错误]:', error);
    ctx.status = 500;
    ctx.body = { success: false, message: '获取热门日记失败：' + error.message };
  }
});
...
router.get('/square/latest', optionalAuth, async (ctx) => {
  try {
    ...
    console.error('[获取最新日记错误]:', error);
    ctx.status = 500;
    ctx.body = { success: false, message: '获取最新日记失败：' + error.message };
  }
});
 ...
 *   get:
 *     summary: 获取关注用户的日记
 *     description: 获取当前用户关注的用户发布的公开日记
 *     security:
 *       - BearerAuth: []
 *     parameters:
 *       - in: query
...
router.get('/square/following', auth, async (ctx) => {
  try {
    ...
    console.error('[获取关注用户日记错误]:', error);
    ctx.status = 500;
    ctx.body = { success: false, message: '获取关注用户日记失败：' + error.message };
  }
});
 ...
 *   get:
 *     summary: 获取同城日记
 *     description: 获取与当前用户同城市的用户发布的公开日记
 *     security:
 *       - BearerAuth: []
 *     parameters:
 *       - in: query
...
router.get('/square/local', auth, async (ctx) => {
  try {
    ...
    // 过滤出同城日记
    const localDiaries = diaries.filter(diary => 
      diary.user_id.city && diary.user_id.city === currentUser.city
    );

    // 处理返回数据
    const diaryList = await Promise.all(localDiaries.map(async (diary) => {
      const isLiked = await Like.exists({ user_id: user._id, diary_id: diary._id });
      const isFollowed = await Follow.exists({ follower_id: user._id, following_id: diary.user_id._id });
    ...
    console.error('[获取同城日记错误]:', error);
    ctx.status = 500;
    ctx.body = { success: false, message: '获取同城日记失败：' + error.message };
  }
});
...
router.post('/square/like', auth, async (ctx) => {
  try {
    ...
    diary.like_count += 1;
    // 更新热度评分
    diary.score = diary.like_count * 2 + diary.comment_count * 3 + diary.share_count * 5;
    await diary.save();
 ...
 *   get:
 *     summary: 获取日记详情
 *     description: 获取指定日记的详细信息，包括作者信息、互动数据、评论列表和相关推荐
 *     security:
 *       - BearerAuth: []
 *     parameters:
 *       - in: path
...
router.post('/square/comment/like', auth, async (ctx) => {
  try {
    ...
    // 检查是否已经点赞
    const existingLike = await CommentLike.findOne({ user_id: user._id, comment_id });
    if (existingLike) {
      ctx.body = { success: false, message: '已经点赞过了' };
      return;
    }
...
router.post('/square/comment/unlike', auth, async (ctx) => {
  try {
    ...
    // 检查点赞记录是否存在
    const like = await CommentLike.findOneAndDelete({ user_id: user._id, comment_id });
    if (!like) {
      ctx.status = 404;
      ctx.body = { success: false, message: '未找到点赞记录' };
      return;
    ...
    ctx.status = 500;
    ctx.body = { success: false, message: '取消评论点赞失败：' + error.message };
  }
});
